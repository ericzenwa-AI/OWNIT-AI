...
def build_prompt(
    *, question: str, verbatim: bool, gap_id: str, entry_id: str, chain: list[str]
    ...
    return (
        ...
        f"- Question {RUNGS} IS their question, put as multiple choice. This "
        "one is the point of the whole ladder and has the strictest rule: it "
        "must give away NOTHING of the working. Restate only what the "
        "original question itself states. Do not carry over any value an "
        "earlier step worked out. Do not compute anything for them, however "
        "small. Do not add a clause like \"so y = 9\" or \"using the "
        "result above\", and do not refer to the earlier questions at "
        "all. If the original says a point has x-coordinate -2, that is what "
        "it says - it does not say what the y-coordinate turns out to be. "
        "This step exists to find out whether they can now do the thing "
        "unaided, and one figure handed to them is the difference between "
        "finding that out and not.\n"
