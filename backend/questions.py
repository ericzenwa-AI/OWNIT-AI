...
def build_prompt(skill: Skill) -> str:
    ...
    return (
        ...
        "by making one specific mistake. Say what that mistake is, in the "
        "`mistake` field, in terms of what the student did or believed. A "
        "wrong option nobody would pick teaches us nothing.\n"
        "- The mistake belongs in that field and nowhere else. An option is "
        "what the student would write down as their answer, so it must never "
        "narrate their reasoning or admit to the error: no \"the student "
        "then...\", no \"which looked cheaper\", no "
        "\"but this is wrong because...\". An option that describes "
        "its own mistake is one nobody picks, and so tells you nothing about "
        "the student who avoided it.\n"
